var tools = function ()
{
	...
	{
		var bacteriaBox = document.getElementById("bacteria"),
			boxStyle = window.getComputedStyle(bacteriaBox);
		var boxW = boxStyle.width.toString(); 
		var boxH = boxStyle.height.toString(); 
		w = boxW.slice(0, -2); w = parseInt(w);
		h = boxH.slice(0, -2); h = parseInt(h);
		this.canvas = document.createElement("canvas");
		if(addToBody || addToBody == undefined)
		{
			var body = document.getElementsByTagName('body')[0];
			
			bacteriaBox.appendChild(this.canvas);
		}
